...
class New extends React.Component {
  ...
  handleChange = (e) => {
    if (e.target.name === "paid") {
      this.setState({ paid: e.target.value === "true" });
    } else {
       this.setState({
    ...
    return(
      <Styles>
        <div className="container">
          ...
          <br/>
          <br/>
          <Form onSubmit={(event) => this.props.handleNewPost(event, this.props, this.state)}>
            <Row>
              <Col>
                <Form.Group className="mb-3" controlId="opportunityDropdown">
                  <Form.Label>Type of Opportunity</Form.Label>
                  <Form.Select onChange={this.handleChange} name="performance_type">
